...
function run_ui(event, page)
    ...
    lastRunTS = now
    if page ~= nil and Page and (currentPage ~= page) then
      setPage(page)
    end
    if (currentState == pageStatus.saving) then
    ...
    elseif userEvent.press.pageDown and ((event == userEvent.longPress.enter) or (event == userEvent.press.menu)) then -- Horus
        menuActive = 1
        if event == userEvent.longPress.enter then
          killEnterBreak = 1
        end
        currentState = pageStatus.displayMenu
    ...
    elseif currentState <= pageStatus.display then
        if event == userEvent.press.pageUp then
            incPage(-1)
        elseif (userEvent.press.pageDown == nil and event == userEvent.release.menu) or event == userEvent.press.pageDown then
            incPage(1)
        elseif event == userEvent.release.plus or event == userEvent.dial.left then
            incLine(-1)
        elseif event == userEvent.release.minus or event == userEvent.dial.right then
        ...
        elseif userEvent.press.pageDown == nil and event == userEvent.release.exit then
